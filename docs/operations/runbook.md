...
KVKK anonymization. Migrations are applied with the owner connection (`bun run db:migrate`, or on boot when
`DATABASE_MIGRATION_URL` is set); the server itself connects as the least-privilege role `ralo_app`. In development, when
...
| `DATABASE_URL` | Postgres connection string. Empty = local PGlite database in `data/pglite` (development only). Server refuses to start in production without it | Render (secret); `.env` locally | Supabase **session pooler** URL with user `ralo_app.<ref>` (see section 6) |
...
| `DATABASE_MIGRATION_URL` | Owner connection (`postgres.<ref>`) used by `bun run db:migrate` and `bun run admin:grant` from a local shell. Falls back to `DATABASE_URL`. If set on the server, migrations are applied on boot with it | local shell (optionally Render secret) | session pooler URL, port `5432`, user `postgres.<ref>` |
...
| `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` | Web push (phone notifications). Generate the key pair once (`node -e "console.log(require('web-push').generateVAPIDKeys())"`); changing it invalidates every device subscription. Empty = push off | Render (secret) | generated once; subject `mailto:<support address>` |
...
1. **Supabase**: create the project (section 6), copy the session pooler connection string. From a local shell,
   run `bun run db:migrate` with `DATABASE_MIGRATION_URL` (user `postgres.<ref>`), then enable the runtime role
   in the SQL editor: `ALTER ROLE ralo_app WITH LOGIN PASSWORD '...';` (section 6).
...
4. Fill the secret env vars: `DATABASE_URL` (user `ralo_app.<ref>`), `DATABASE_SSL_CA`, `APP_URL`, `SMTP_USER`,
...
6. On boot the server checks that every migration is applied (`Schema is up to date ...`) and refuses to start
   otherwise. An empty production database stays empty: demo data is only seeded when `DEMO_MODE=true`.
7. Verify (section 7).
...
- **Runtime role (`ralo_app`)**: the server must **not** connect as `postgres`. The `postgres` user owns
  the tables, so row level security (the tenant backstop that keeps one club's rows away from another
  club's panel) does not apply to it, and a bug or SQL injection would have full rights. Migrations
  create `ralo_app` without a password (NOLOGIN); enable it once per project in the Supabase SQL editor:
  ```sql
  ALTER ROLE ralo_app WITH LOGIN PASSWORD '<long random password from the password manager>';
  ```
  - `DATABASE_URL` on Render: the session pooler URL with user **`ralo_app.<project-ref>`** and that password.
  - `DATABASE_MIGRATION_URL`: the same URL with user `postgres.<project-ref>`. Keep it in your local shell
    and run `bun run db:migrate` **before** deploying a release that adds a migration. A server connected as
    `ralo_app` cannot change the schema; it refuses to start with `Database schema is not up to date: ...`
    while a migration is missing. (Alternatively set `DATABASE_MIGRATION_URL` on Render too; the server then
    applies migrations on boot, at the cost of keeping the owner password on Render.)
  - Boot log with `ralo_app`: `Schema is up to date (runtime role without schema privileges ...)`.
  - The API sets the request scope itself (`global` by default, `club:<id>` inside club panel transactions);
    CI runs the whole test suite as `ralo_app` on Postgres 17.
...
3. Render logs for the new instance contain, in order:
   - `Schema is up to date (runtime role without schema privileges ...)` (or `Applied NNNN_<name>.sql` lines when
     `DATABASE_MIGRATION_URL` is set on the server)
   ...
   `Database schema is not up to date:` means `bun run db:migrate` was not run before the deploy.
...
**Run migrations** before deploying a release that adds one (the server applies them on boot only when `DATABASE_MIGRATION_URL` is also set on Render):
...
| `booking_suspensions` | once a day | turns off app bookings for clubs with a statement unpaid 7 days past due (re-opened automatically when paid) |
| `error_events_purge` | once a day | deletes error records older than 30 days |
| `match_reminders` + `push_dispatch` | every minute | creates 2-hour match reminders and sends new notifications to subscribed devices (web push) |

Errors: `/admin` > Hatalar lists unexpected server errors and browser errors grouped by message, without personal
data. Also add an external uptime monitor (e.g. a free UptimeRobot check on `/api/health`) so someone is alerted
when the whole service is down.
...
- **Reviews:** players with a finished reservation or lesson session at a club rate it (1-5, one review each);
  admins remove abusive reviews on the club page. Reviews are deleted with the author's account.
- **Photos:** club cover and up to 6 photos per court, uploaded by owners (panel > Kort Yönetimi) or admins; stored
  in `app.media_assets` (resized in the browser, max 700 KB) and served from `/api/media/<id>`.
